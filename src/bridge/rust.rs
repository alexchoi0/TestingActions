...
impl RustBridge {
    ...
    async fn cargo_build(bin_name: &str, working_dir: Option<&str>) -> Result<String, BridgeError> {
        ...
        let status = cmd
            .status()
            .await
            .map_err(|e| BridgeError::StartupFailed(format!("Failed to run cargo build: {}", e)))?;

        if !status.success() {
            return Err(BridgeError::StartupFailed(format!(
                "cargo build --bin {} failed",
                bin_name
            )));
        }
        ...
        let target_dir = metadata
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                BridgeError::StartupFailed("No target_directory in metadata".to_string())
            })?;
...
#[async_trait]
impl super::Bridge for RustBridge {
    fn platform(&self) -> crate::workflow::Platform {
        crate::workflow::Platform::Rust
    }
    ...
    async fn sync_clock(&self, state: &ClockSyncState) -> Result<(), BridgeError> {
        RustBridge::sync_clock(self, state).await
    }
...
mod tests {
    ...
    fn test_rust_config_with_binary() {
        ...
        assert_eq!(
            config.binary,
            Some("./target/debug/test-registry".to_string())
        );
    }
